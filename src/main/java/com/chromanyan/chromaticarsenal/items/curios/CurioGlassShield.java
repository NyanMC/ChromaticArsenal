...
public class CurioGlassShield extends BaseCurioItem {
    ...
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> list, @NotNull TooltipFlag flag) {
        ...
        list.add(new TranslatableComponent("tooltip.chromaticarsenal.glass_shield.2", "§b" + (((float) getCooldownDuration(stack)) / 20)));
        if (getFreeBlockChance(stack) > 0) {
            list.add(new TranslatableComponent("tooltip.chromaticarsenal.glass_shield.3", "§b" + getFreeBlockChance(stack)));
        }
